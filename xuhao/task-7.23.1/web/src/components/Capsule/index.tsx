    ...
    render(){
        ...
        return(
            <div className={style.capsule}>
                <p className={style.capsule_title}><span className={style.title_name}>【{_this.state.name}】</span>在<span className={style.title_time}>【{_this.state.time}】</span>写下的时间胶囊</p>
                <p className={style.capsule_content}>{(new Date().getTime()>_this.state.seconds)?_this.state.content:`还没到时间，距离可以开启还有${_this.state.count}秒`}</p>
            </div>
        )
    }
}